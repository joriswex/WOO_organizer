...
Both pipelines run three steps in sequence.
...
- **No stamps anywhere** → auto-split mode:
  - *Default:* `_build_docs_auto_split()` uses heuristics — `within_doc_page == 1`, `"Pagina 1 van"` in text, or a fresh email header block (≥3 header fields in the first 20 lines) appearing after non-email content. Documents receive `auto_001`, `auto_002`, … codes.
...
Each email gets an ID like `0003.2` and carries `subject`, `sender`, `to`, `cc`, and `date` fields extracted from its header lines.
